...
description: 'Converts Word (.docx) documents into Markdown so their contents can be accurately analyzed, summarized, searched, or extracted from. Use whenever a .docx file must be read or used as PPT source material. Always run the bundled conversion script first; do not parse .docx ad hoc. This portable workspace bundles DOCX support only: report PDF or Excel files as unsupported unless their corresponding conversion Skills are actually available.'
---
...
**Mixed file types:** This portable workspace bundles only DOCX conversion.
List every detected file type. Convert `.docx` files with this Skill; process
PDF or Excel only when their corresponding Skills are actually installed.
Otherwise report those files as unsupported before continuing. Never skip a
file type silently and never pretend an unavailable sibling Skill was invoked.
...
"save the output to `<output-directory>`", "put the results in `<project>/input/.converted`"). Do NOT
...
ask the user to confirm the full path before running the
